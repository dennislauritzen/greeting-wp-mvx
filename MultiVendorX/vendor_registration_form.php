...
if (!empty($mvx_vendor_registration_form_data) && is_array($mvx_vendor_registration_form_data)) {
    ...
    foreach ($mvx_vendor_registration_form_data as $key => $value) {
        switch ($value['type']) {
            ...
            case 'textbox':
                ?>
                <div class="mb-3 <?php if(!empty($value['cssClass'])){ echo $value['cssClass']; } ?>">
                    <label class="form-label"><?php echo __($value['label'],'dc-woocommerce-multi-vendor'); ?><?php if($value['required']){ echo ' <span class="required">*</span>'; }?>
                    ...
                    <input type="text" class="form-control" value="<?php if (!empty($form_data['mvx_vendor_fields'][$key]["value"])) echo esc_attr($form_data['mvx_vendor_fields'][$key]["value"]); ?>" name="mvx_vendor_fields[<?php echo $key; ?>][value]" placeholder="<?php echo $value['placeholder']; ?>" <?php if($value['required']){ echo 'required="required"'; }?> />
            ...
            case 'email':
                ?>
                <div class="mb-3 <?php if(!empty($value['cssClass'])){ echo $value['cssClass']; } ?>">
                    <label class="form-label"><?php echo __($value['label'],'dc-woocommerce-multi-vendor'); ?><?php if($value['required']){ echo ' <span class="required">*</span>'; }?></label>
                    <input type="email" class="form-control" value="<?php if (!empty($form_data['mvx_vendor_fields'][$key]["value"])) echo esc_attr($form_data['mvx_vendor_fields'][$key]["value"]); ?>" name="mvx_vendor_fields[<?php echo $key; ?>][value]" placeholder="<?php echo $value['placeholder']; ?>" <?php if($value['required']){ echo 'required="required"'; }?> />
                    <input type="hidden" name="mvx_vendor_fields[<?php echo $key; ?>][label]" value="<?php echo htmlentities($value['label']); ?>" />
            ...
            case 'textarea':
                ...
                <div class="mb-3 <?php if(!empty($value['cssClass'])){ echo $value['cssClass']; } ?>">
                    ...
                    <textarea class="form-control" <?php if(!empty($value['limit'])){ echo 'maxlength="'.$value['limit'].'"'; } ?> name="mvx_vendor_fields[<?php echo $key; ?>][value]" placeholder="<?php echo $value['defaultValue']; ?>" <?php if($value['required']){ echo 'required'; }?>><?php if (!empty($form_data['mvx_vendor_fields'][$key]["value"])){ echo esc_attr($form_data['mvx_vendor_fields'][$key]["value"]); } ?></textarea>
                    <input type="hidden" name="mvx_vendor_fields[<?php echo $key; ?>][label]" value="<?php echo htmlentities($value['label']); ?>" />
            ...
            case 'url':
                ?>
                <div class="mb-3 <?php if(!empty($value['cssClass'])){ echo $value['cssClass']; } else {  echo 'wcmp-regi-12'; } ?>">
                    <label class="form-label"><?php echo __($value['label'],'dc-woocommerce-multi-vendor'); ?><?php if($value['required']){ echo ' <span class="required">*</span>'; }?></label>
                    <input type="url" class="form-control" value="<?php if (!empty($form_data['mvx_vendor_fields'][$key]["value"])) echo esc_attr($form_data['mvx_vendor_fields'][$key]["value"]); ?>" name="mvx_vendor_fields[<?php echo $key; ?>][value]" placeholder="<?php echo $value['placeholder']; ?>" <?php if($value['required']){ echo 'required="required"'; }?> />
                    <input type="hidden" name="mvx_vendor_fields[<?php echo $key; ?>][label]" value="<?php echo htmlentities($value['label']); ?>" />
                ...
                <div class="mb-3 <?php if(!empty($value['cssClass'])){ echo $value['cssClass']; } else {  echo 'wcmp-regi-12'; } ?>">
                    ...
                    <?php
                     switch ($value['selecttype']){
                         case 'dropdown':
                            ?>
                            <select class="form-select" name="mvx_vendor_fields[<?php echo $key; ?>][value]" <?php if($value['required']){ echo 'required="required"'; }?>>
                            <?php
                         ...
                         case 'radio':
                             if (!empty($value['options']) && is_array($value['options'])) {
                                ...
                                foreach ($value['options'] as $option_key => $option_value) {
                                    ?>
                                    <p><input type="radio" class="form-control" <?php if($option_value['selected']){ echo 'checked="checked"'; } ?> name="mvx_vendor_fields[<?php echo $key; ?>][value]" value="<?php echo $option_value['value']; ?>" <?php if($value['required']){ echo 'required="required"'; }?>> <?php echo $option_value['label']; ?></p>
                                    <?php
                                }
                                ?>
                                </div>
                         ...
                         case 'checkboxes':
                            if (!empty($value['options']) && is_array($value['options'])) {
                                ...
                                foreach ($value['options'] as $option_key => $option_value) {
                                    ...
                                    ?>
                                    <p>
																			<input type="checkbox" id="mvx_vendor_check_multi<?php echo $i; ?>" class="form-check-input" <?php if($option_value['selected']){ echo 'checked="checked"'; } ?> name="mvx_vendor_fields[<?php echo $key; ?>][value]" value="<?php echo $option_value['value']; ?>" <?php if($value['required']){ echo 'required="required"'; }?>>
																			<label for="mvx_vendor_check_multi<?php echo $i; ?>" class="form-check-label"><?php echo $option_value['label']; ?></label>
                                    </p>
                         ...
                         case 'multi-select':
                             ?>
                            <select class="form-select" style="min-height: 59px;" name="mvx_vendor_fields[<?php echo $key; ?>][value][]" <?php if($value['required']){ echo 'required="required"'; }?> multiple="">
                            <?php
                            if (!empty($value['options']) && is_array($value['options'])) {
                                foreach ($value['options'] as $option_key => $option_value) {
                                    ?>
            ...
            case 'file':
                ...
                <div class="mb-3  <?php if(!empty($value['cssClass'])){ echo $value['cssClass']; } else {  echo 'wcmp-regi-12'; } ?>">
                    <label class="form-label"><?php echo __($value['label'],'dc-woocommerce-multi-vendor'); ?><?php if($value['required']){ echo ' <span class="required">*</span>'; }?></label>
                    <input type="file" class="form-control" name="mvx_vendor_fields[<?php echo $key; ?>][]" <?php if($value['required']){ echo 'required="required"'; }?> <?php if($value['muliple']){ echo 'multiple="true"'; }?> />
                    <input type="hidden" name="mvx_vendor_fields[<?php echo $key; ?>][label]" value="<?php echo htmlentities($value['label']); ?>" />
